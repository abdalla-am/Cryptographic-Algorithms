    ...
    public class RC4 : CryptographicTechnique
    {
        ...
        public override string Encrypt(string plainText, string key)
        {
            ...
            if (plainText[1] == 'x' && plainText[0] == '0')
            {
                ...
                {
                    temp += char.ConvertFromUtf32(Convert.ToInt32(plainText[i].ToString() + plainText[i + 1].ToString(), 16));
                }
                plainText = temp;
            }
            if (key[0] == '0' && key[1] == 'x')
            ...
            while(counter < plainText.Length )
            {
                ...
                t = (S[a] + S[l]) % 256;
                k = S[t];
                Console.WriteLine(plainText[counter].ToString() + k.ToString());
                C += char.ConvertFromUtf32((plainText[counter] ^ k));
                counter++;
            }
